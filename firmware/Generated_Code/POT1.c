...
**          Wait for result                                : yes
**     Contents    :
**         Measure    - byte POT1_Measure(bool WaitForResult);
**         GetValue16 - byte POT1_GetValue16(word *Values);
**         Calibrate  - byte POT1_Calibrate(bool WaitForResult);
**
...
*/         
/*!
**  @addtogroup POT1_module POT1 module documentation
**  @{
*/         
...
static LDD_ADC_TSample SampleGroup[POT1_SAMPLE_GROUP_SIZE];
/* Measure multiple channels flags  */
/* Temporary buffer for converting results */
volatile word POT1_OutV[POT1_SAMPLE_GROUP_SIZE]; /* Sum of measured values */
...
{
  POT1_OutV[0] = 0U;                   /* Set variable for storing measured values to 0 */
  POT1_OutV[1] = 0U;                   /* Set variable for storing measured values to 0 */
}

/*
** ===================================================================
**     Method      :  POT1_HWEnDi (component ADC)
**
**     Description :
...
/*
** ===================================================================
**     Method      :  POT1_Measure (component ADC)
**     Description :
...
/*
** ===================================================================
**     Method      :  POT1_GetValue16 (component ADC)
**     Description :
...
** ===================================================================
*/
byte POT1_GetValue16(word *Values)
{
  if (!OutFlg) {                       /* Is output flag set? */
    return ERR_NOTAVAIL;               /* If no then error */
  }
  Values[0] = POT1_OutV[0];            /* Save measured values to the output buffer */
  Values[1] = POT1_OutV[1];            /* Save measured values to the output buffer */
  return ERR_OK;                       /* OK */
}

/*
** ===================================================================
**     Method      :  POT1_Calibrate (component ADC)
**     Description :
...
** ===================================================================
*/
byte POT1_Calibrate(bool WaitForResult)
{
  if (ModeFlg != STOP) {               /* Is the device in different mode than "stop"? */
    return ERR_BUSY;                   /* If yes then error */
  }
...
/*
** ===================================================================
**     Method      :  POT1_OnMeasurementComplete (component ADC)
**
**     Description :
...
void AdcLdd1_OnMeasurementComplete(LDD_TUserData *UserDataPtr)
{
  ...
  if (ModeFlg == CALIBRATING) {        /* If the driver is in CALIBRATING mode */
    ...
    ModeFlg = STOP;                    /* Set the device to the stop mode */
    POT1_OnCalibrationEnd();           /* If yes then invoke user event */
    return;                            /* Return from interrupt */
  }
  AdcLdd1_GetMeasuredValues(AdcLdd1_DeviceDataPtr, (LDD_TData *)&POT1_OutV[SumChan]);
  ...
  if (SumChan == 2U) {                 /* Is number of measured channels equal to the number of channels used in the component? */
    ...
    POT1_OnEnd();                      /* If yes then invoke user event */
    ModeFlg = STOP;                    /* Set the device to the stop mode */
    return;                            /* Return from interrupt */
  }
...
/*
** ===================================================================
**     Method      :  POT1_Init (component ADC)
**
**     Description :
